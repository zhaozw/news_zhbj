...
import cn.itcast.zz.zhbj20.utils.MyBitmapUtils;
...
//        bitmapUtils = new BitmapUtils(context);
    ...
    private void parseJson(String resultJson) {
        ...
        gv_arr_pic.setAdapter(adapter);//gv与lv的子条目一样,公用适配器
    }

    private boolean isListType = true;//默认是listview

    /**
     * 传递 右侧按钮
     * @param imgbtn_right2
     */
    public void passBtn(final ImageButton imgbtn_right2) {
        //点击按钮 -- 切换 lv/gv
        imgbtn_right2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(isListType){
                    ...
                    imgbtn_right2.setImageResource(R.drawable.icon_pic_grid_type);
                }else{
                    //切换为 lv
...
//            bitmapUtils.display(hodler.iv_arr_pic_item,NetUrl.base_url+arrPicBean.listimage);
            myBitmapUtils.display(hodler.iv_arr_pic_item,NetUrl.base_url+arrPicBean.listimage);
            return convertView;
        }
    }
    class ArrpicHodler{
